/** The vendored mono marks, transcribed from `assets/*.svg` (spec 12.1).
 *
 * Transcribed rather than injected as raw markup, which the repo's own TS security detector
 * refuses; `tests/graph/test_runner_marks.py` is what keeps this copy identical to its source.
 */
...
export function markFor(runner: string): Mark | null {
...
export default function RunnerMark({ runner, size = 13 }: { runner: string; size?: number }) {
  ...
  return (
    ...
    >
      ...
      <path clipRule={mark.clipRule} d={mark.d} />
    </svg>
  );
}
