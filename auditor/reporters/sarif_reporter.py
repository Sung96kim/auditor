...
class SarifReporter(Reporter):
    ...
    LEVELS: ClassVar[dict[Severity, str]] = {
            ...
            "runs": [
                {
                    "tool": {
                        "driver": {
                            ...
                            "rules": [
                                self.rule_descriptor(rid) for rid in sorted(rule_ids)
                            ],
                        }
                    },
                    "results": [
                        self.result(r.file, f) for r in results for f in r.findings
                    ],
                }
            ],
        }
    ...
    def rule_descriptor(self, rule_id: str) -> dict:
    ...
    def result(self, file: str, f: Finding) -> dict:
        return {
            "ruleId": f.rule_id,
            "level": self.LEVELS.get(f.severity, "warning"),
            "message": {"text": f.message},
            # line-independent fingerprint (file + rule + offending text) so code-scanning dedupes
            # the same finding across runs/commits even when surrounding lines shift
            "partialFingerprints": {
                "auditorFingerprint/v1": finding_fingerprint(file, f)
            },
            "locations": [
                {
                    "physicalLocation": {
