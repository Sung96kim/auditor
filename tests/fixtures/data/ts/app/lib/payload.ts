/* Inert detector fixture — NOT a working program and never invoked. Each line below is an
   isolated specimen that trips exactly one TS-MAL rule's static signal; every network target
   is an RFC-reserved non-resolving host (`*.invalid`), no file is ever opened (the credential
   path is a bare string), and nothing here executes. Lives under `src/` so the malware
   category stays at production strength (never relaxed). */
