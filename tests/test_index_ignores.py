...
async def test_remove_by_id_and_by_selector(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        i1 = await s.add_ignore("PY-A", "a.py", None, None, None, 1.0)
        await s.add_ignore("PY-B", None, None, None, None, 1.0)
        assert await s.remove_ignore_by_id(i1) is True
...
async def test_ignores_survive_schema_version_rebuild(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        await s.add_ignore("PY-X", "a.py", 5, "ev", "keep me", 1.0)
