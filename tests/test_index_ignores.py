...
async def test_add_list_roundtrip(tmp_path):
    ...
    async with await IndexStore.connect(db, "/r") as s:
        rid = await s.ignores.add("PY-X", "a.py", 5, "ev", "why", 1.0)
...
async def test_add_is_idempotent_per_scope(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        first = await s.ignores.add("PY-X", "a.py", 5, "ev1", "r1", 1.0)
        second = await s.ignores.add("PY-X", "a.py", 5, "ev2", "r2", 2.0)  # same scope
        rows = await s.ignores.list()
...
async def test_distinct_scopes_are_separate_rows(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        await s.ignores.add("PY-X", None, None, None, None, 1.0)  # repo-wide
        await s.ignores.add("PY-X", "a.py", None, None, None, 1.0)  # file-wide
        await s.ignores.add("PY-X", "a.py", 5, "ev", None, 1.0)  # line-level
        rows = await s.ignores.list()
    assert len(rows) == 3
...
async def test_remove_by_id_and_by_selector(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        i1 = await s.ignores.add("PY-A", "a.py", None, None, None, 1.0)
        await s.ignores.add("PY-B", None, None, None, None, 1.0)
        assert await s.ignores.remove_by_id(i1) is True
        assert await s.ignores.remove_by_id(9999) is False
        assert await s.ignores.remove_by_selector("PY-B", None, None) is True
        assert await s.ignores.remove_by_selector("PY-B", None, None) is False
...
async def test_clear(tmp_path):
    db = tmp_path / "index.db"
    async with await IndexStore.connect(db, "/r") as s:
        await s.ignores.add("PY-A", None, None, None, None, 1.0)
        await s.ignores.add("PY-B", "a.py", None, None, None, 1.0)
        assert await s.ignores.clear() == 2
        assert await s.ignores.list() == []
...
async def test_ignores_are_repo_partitioned(tmp_path):
    ...
    async with await IndexStore.connect(db, "/repos/a") as a:
        await a.ignores.add("PY-X", None, None, None, None, 1.0)
    async with await IndexStore.connect(db, "/repos/b") as b:
        await b.ignores.add("PY-Y", None, None, None, None, 1.0)
