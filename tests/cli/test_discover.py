...
def test_discover_config_json_invalid_value_errors(tmp_path):
    """discover --config-json with a bad value type → exit 1, 'invalid config' in output."""
    (tmp_path / "pyproject.toml").write_text('[project]\nname="x"\nversion="0"\n')
    result = invoke(
        "discover", str(tmp_path), "--config-json", '{"respect_gitignore": "nope"}'
    )
